...
<body class="bg-light">
    ...
    <!--Edit Data pemesan-->
                ...
                try {
                    ...
                    $stmt = $pdo->prepare("SELECT * FROM pemesan WHERE id_pemesan=?");
                    $stmt->execute([$_GET["id_pemesan"]]);
                    $row = $stmt->fetch();
                    ...
                    if(!$row){
                        ...
                        <div class="pt-3 ms-3 mb-2 fw-bold pb-2" style="border-bottom: 1px solid #e5e5e5;">Edit pemesan</div>
                            <form class="row g-3 ps-3 pe-2 pb-5" action="edit_pelanggan_action.php" method="POST">
                                <div class="col-12">
                                    <label for="inputAddress" class="form-label">ID pemesan</label>
                                    <input type="text" class="form-control" id="id_pemesan" name="id_pemesan" value="<?php echo $row["id_pemesan"]; ?>" required>
                                </div>
                                <div class="col-md-6">
